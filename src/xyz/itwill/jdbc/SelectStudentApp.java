...
public class SelectStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			//ResultSet : 검색 결과를 테이블(2차원 배열) 형식으로 저장한 인스턴스
			// => Result 인터페이스의 참조변수에 자식 클래스의 인스턴스를 반환받아 저장
			rs = stmt.executeQuery(sql);
			
			//ResultSet 인스턴스에 저장된 검색행을 행단위로 처리하기 위해 내부적인 커서(Cursor) 제공
			// => ResultSet Cursor는 최초 BOF(Before Of File) 영역에 위치
