...
//JDBC 프로그램을 작성하기 위한 JDBC Driver 관련 클래스가 포함된 라이브러리 빌드 처리
...
public class InsertStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			//JDBC Driver : DriverManager 클래스의 등록된 다수의 Driver 인스턴스
			// => Driver 인스턴스 : DBMS 서버에 접속할 수 있는 기능 제공
			// => URL 주소로 DBMS 서버에 접속할 수 있는 기능 제공
			...
			*/
			...
			//OracleDriver 클래스를 읽어 메모리에 저장 - *권장*
			// => OracleDriver 클래스의 정적영영에서 OracleDriver 클래스를 인스턴스로 
			...
			// => DBMS에서 접속 오류 또는 SQL 명령에 문제가 있는 경우 예외 발생
			...
			//String sql = "insert into student values(3000,'임꺽정','010-1115-5123','수원시 팔달구','1969-12-31')";
			String sql = "insert into student values(4000,'전우치','010-2875-5123','인천시 월미도','1970-12-31')";
			int rows = stmt.executeUpdate(sql);
			
			//5.SQL 명령의 실행 결과를 처리 - 출력 또는 반환
			System.out.println("[메시지]" + rows + "명의 학생정보를 저장 하였습니다.");
